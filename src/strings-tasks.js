...
function formatTime(minutes, seconds) {
  const minut = minutes.toString().padStart(2, 0);
...
function isPalindrome(str) {
  const strLow = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  const reverse = strLow.split('').reverse().join('');
  return strLow === reverse;
}
...
function findLongestWord(sentence) {
  const long = sentence.split(' ').sort((a, b) => b.length - a.length);
  return long[0];
}
...
function reverseWords(str) {
  ...
  const reverWord = reverse.map((word) => word.split('').reverse().join(''));
...
function encodeToRot13(str) {
  ...
  return str.replace(/[A-Za-z]/g, (letter) => {
    const ind = input.indexOf(letter);
    return output[ind];
  });
}
